...
- In case of RDS Multi-AZs, you don't need to change the connection setting as AWS itself will failover the RDS instance in different AZ w/o changing the connection string.
- Multi AZ is for disaster recovery.
Multi AZ is not externally supported by only one DB Aurora, as that itself is Fault-Tolerant.
- Multi AZ is synchronus.
- Read Replicas(RRs) are asynchronus.
- Your EC2 will be reading from Read Replicas and writting to only one primary instance. These writes then later will be replicated to RRs asynchronously.
- To be RR effective, you need to have the back up turned on.